...
function updateSitemap(slugs) {
  ...
  // Preserve existing <lastmod> values so unchanged /wordle/ URLs keep their
  // original date; only brand-new URLs get today's date. Without this, every
  // run re-dates every wordle URL on the site, not just the new ones.
  const existing = {};
  ...
  const newEntries = slugs.map(slug => {
    const loc = `${SITE_URL}/wordle/${slug}.html`;
